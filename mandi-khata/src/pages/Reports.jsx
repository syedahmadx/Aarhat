...
import { formatRs, todayISO, daysAgoISO, formatDate } from '../utils/format';
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">Sirf Malik ke liye</p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
        <div>
          ...
          <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-primary-500" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold text-gray-600">To</label>
          <input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none focus:border-primary-500" />
        </div>
        {invalidRange && <p className="pb-2 text-sm font-medium text-red-600">"From" date "To" ke baad nahi ho sakti</p>}
      </div>

      {!data ? null : data.saleCount === 0 && data.cashIn === 0 && data.cashOut === 0 ? (
        <EmptyState title="Is range mein koi data nahi" subtitle="Date range tabdeel kar ke dobara dekhein." />
      ) : (
        <>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium text-gray-500">Total Sales ({data.saleCount})</p>
              <p className="mt-1 text-lg font-bold text-gray-900">{formatRs(data.totalSales)}</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium text-gray-500">Commission Earned</p>
              <p className="mt-1 text-lg font-bold text-amber-700">{formatRs(data.commission)}</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium text-gray-500">Cash In</p>
              <p className="mt-1 text-lg font-bold text-green-700">{formatRs(data.cashIn)}</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium text-gray-500">Cash Out</p>
              <p className="mt-1 text-lg font-bold text-red-600">{formatRs(data.cashOut)}</p>
            </div>
          </div>

          <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="text-base font-bold text-gray-900">Daily Sales</h2>
            ...
            <div className="mt-5 flex items-end gap-2 overflow-x-auto pb-2" style={{ height: '180px' }}>
              {data.days.map((d) => (
                <div key={d.date} className="flex h-full min-w-10 flex-1 flex-col items-center justify-end gap-1">
                  <span className="text-[10px] font-semibold text-gray-600">{d.total > 0 ? formatRs(d.total).replace('Rs. ', '') : ''}</span>
                  <div
                  ...
                  />
                  <span className="whitespace-nowrap text-[10px] text-gray-400">{formatDate(d.date).slice(0, 6)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <h2 className="mb-3 text-base font-bold text-gray-900">Kharcha by Type</h2>
            {Object.keys(data.expensesByType).length === 0 ? (
              <p className="text-sm text-gray-500">Is range mein koi kharcha nahi.</p>
            ) : (
                ...
                {Object.entries(data.expensesByType)
                  ...
                  .map(([type, amount]) => (
                    <li key={type} className="flex items-center gap-3">
                      <span className="w-28 shrink-0 text-sm font-medium text-gray-700">{type}</span>
                      <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-gray-100">
                        <div className="h-full rounded-full bg-primary-500" style={{ width: `${(amount / data.totalExpenses) * 100}%` }} />
                      </div>
                      <span className="w-24 shrink-0 text-right text-sm font-semibold text-gray-900">{formatRs(amount)}</span>
                    </li>
                  ))}
                <li className="flex items-center justify-between border-t border-gray-100 pt-2.5">
                  <span className="text-sm font-bold text-gray-900">Total Kharcha</span>
                  <span className="text-sm font-bold text-red-600">{formatRs(data.totalExpenses)}</span>
                </li>
              </ul>
            )}
          </div>
